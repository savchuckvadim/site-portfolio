    ...
    return (
        <>
            <div
                ...
                className={`cursor-pointer text-foreground ${isThemeSpinning ? 'animate-spin-fast animate-shake' : ''}`}
            >
                {theme == 'default' ? (
                    <Atom
                    ...
                    <Atom className="h-4 w-4 md:h-6 md:w-6" color="blue" />
                ) : theme == 'violete' ? (
                    <Atom className="h-4 w-4 md:h-6 md:w-6" color="violet" />
                ) : (
                    <Atom
                        className="h-4 w-4 md:h-6 md:w-6"
                        color={mode === 'dark' ? 'white' : 'black'}
            ...
            >
                <div
                    className={`text-foreground ${isSpinning ? 'animate-spin' : ''}`}
                >
                    {mode === 'dark' ? (
                        <MoonStar className="h-4 w-4 md:h-6 md:w-6" size={24} />
                    ) : (
                        <SunDim className="h-4 w-4 md:h-6 md:w-6" size={24} />
                    )}
                </div>
            </div>
        </>
